...
| `status` | `active` / `stale` / `refuted` / `forgotten`. Normal recall sees only `active`. |
...
| `fibbrain_observe(kind, summary, payload)` | What just happened? | no (episode only) |
...
purpose: two trees, four nodes, nine events, one identity.
...
- `queries`: fixed search queries with identity and the exact ordered titles
  they must return — including three that must return nothing (wrong project,
  no identity, unrelated terms);
