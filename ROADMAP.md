...
>
> 未过门（2026-09-18 复测）：`fibmind_hybrid`（hashing 向量 + RRF）在固定数据集上
> R@K 与 `fibmind_current` 持平，但无相关查询正确率 1.0 → 0.5、无关注入 0.18 → 0.24、
> P95 延迟约 4 倍。"混合检索超过关键词基线"这一 v0.2 发布条件目前**没有**数据支持；
> 在真实轨迹数据集到位前，默认保持纯词法召回。`evals/gate.py` 只对 `fibmind_current`
> 设底线，正是因为 hybrid 还没有达到可以设底线的水平。
